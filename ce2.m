...
for j=0:2
%tolFixedPoint = 1e-10;    
tolFixedPoint = 10^(-j*4); %0 -4 -8
init_capU0 = 5*ones(nSuppX,2); 
init_capU1 = 5*ones(nSuppX,2);
[capU0,capU1] = fixedPoint(u0,u1,capPi,rho,tolFixedPoint,@bellman,init_capU0,init_capU1);
deltaU = capU1-capU0;
%{
...
TolFun = 1E-6;
TolX = 1E-10;
%TolFun = 10^(-13+j*6); %-13 -7-1
%TolX = 10^(-9+j*4); %-9 -5 -1
OptimizerOptions = optimset('Display','off','Algorithm','interior-point','AlwaysHonorConstraints','bounds',...
                            'GradObj','on','TolFun',TolFun,'TolX',TolX,'DerivativeCheck','off','TypicalX',[beta;delta(2)]);
...
%{
The resulting parameter estimates and standard errors are displayed
%}
disp('Summary of Results');
disp('---------------------------------------------------------------------------------');
disp('Low TolFun/X High TolFixed        ---->                High TolFun/X Low TolFixed');
disp('   mean_est   ste_est  mean_est   ste_est  mean_est   ste_est');
disp([mean(Estimates1, 1)' std(Estimates1, 1)' mean(Estimates2, 1)' std(Estimates2, 1)' mean(Estimates3, 1)' std(Estimates3, 1)']);
